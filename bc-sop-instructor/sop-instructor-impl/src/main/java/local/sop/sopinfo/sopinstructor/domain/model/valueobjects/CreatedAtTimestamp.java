...
public record CreatedAtTimestamp(LocalDateTime value) {
    public CreatedAtTimestamp {
		if (value == null) {
			throw new ValidationException("sop-instructor.createdate.required",
					Map.of("field", "createdAt", "createdAt", value == null ? "null" : value.toString()));
		}

		LocalDateTime now = LocalDateTime.now();
		if (value.isAfter(now)) {
			throw new ValidationException("sop-instructor.createdate.isafter", Map.of("createdAt", value));
		}
	}
}
