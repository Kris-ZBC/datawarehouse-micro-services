package local.sop.sopinfo.sopeducation.application.architecture;
...
@AnalyzeClasses(packages = { "local.sop.sopinfo.sopeducation",
                             "local.sop.sopinfo.sharedkernel" }, 
    importOptions = {
...
public class ApplicationLayerCleanTest {
    ...
    static final ArchRule api_must_not_depend_on_spring_or_jpa =
        noClasses().that().resideInAnyPackage("..application.api..")
                ...
                .because("The application.api layer should be a pure API layer without dependencies on specific frameworks like Spring or JPA.");
        ...
        .should().onlyDependOnClassesThat(new DescribedPredicate<>("JDK, Jakarta Validation, or same DTO package") {
            @Override public boolean test(JavaClass c) {
                ...
                return c.getName().startsWith("java.")      // JDK (includes java.lang.Record)
                    ...
                    || pkg.contains(".sharedkernel.enums") // shared enums used in DTOs
                    || pkg.contains(".sharedkernel.compositekey.dtos"); // CompositeKey    
            }
        })
        .because("DTO records must be independent of all other modules, except Jakarta Validation, and shared enums.");
